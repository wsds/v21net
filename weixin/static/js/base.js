...
STK.register("core.str.decodeHTML", function(a) {
    return function(a) {
        ...
        return a.replace(/&quot;/g, '"').replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&#39;/g, "'").replace(/&nbsp;/g, "Â ").replace(/&#32;/g, " ").replace(/&amp;/g, "&")
    }
});
...
STK.register("historyM", function(a) {
    ...
    function A(b, c) {
        if (!/^\//.test(b))
            a.log("µØÖ·:(" + b + ") ²»ÎªÕýÈ·µÄÂ·¾¶,ÇëÊ¹ÓÃ¾ø¶ÔÂ·¾¶; Àý£º/directory/page.php?params#hash");
        else {
            ...
            a.core.evt.addEvent(document, "click", function(b) {
                ...
                if (j) {
                    if (g.getAttribute("bpfilter") && g.getAttribute("bpfilter") == h && !b.ctrlKey && !b.shiftKey && b.button == 0 && (!c || !b.metaKey)) {
                        a.log("bigpipeM: µã»÷ÐÐÎªÀ¹½Ø³É¹¦£¬Ê¹ÓÃBigpipeÄ£Ê½¼ÓÔØ");
...
STK.register("ui.alert", function(a) {
    ...
    return function(g, h) {
        var i, j, k, l, m;
        i = a.parseParam({title: d("#L{ÌáÊ¾}"),icon: "warn",textLarge: g,textSmall: "",OK: a.funcEmpty,OKText: d("#L{È·¶¨}"),timeout: 0}, h);
        i.icon = c[i.icon];
        j = {};
...
STK.register("ui.confirm", function(a) {
    ...
    return function(f, g) {
        var h, i, j, k, l, m;
        h = a.parseParam({title: d("#L{ÌáÊ¾}"),icon: "question",textLarge: f,textComplex: "",textSmall: "",OK: a.funcEmpty,OKText: d("#L{È·¶¨}"),cancel: a.funcEmpty,cancelText: d("#L{È¡Ïû}")}, g);
        h.icon = c[h.icon];
        i = {};
        e || (e = a.kit.extra.reuse(function() {
        ...
        if (!c) {
            var e, f = {init: function() {
                ...
                return function(a) {
                    ...
                    a.ui.confirm(b("#L{È·ÈÏÇÐ»»µ½}") + f.innerHTML + b("#L{°æÂð£¿}"), {OK: function() {
                            a.preventDefault();
                            if (e.login)
                                g(c);
                            else {
        ...
        var g = {base64form: null,upload: function(b) {
                ...
                if (!e || e.ret < 0) {
                    ...
                    switch (f) {
                        ...
                        case 4:
                        case 9:
                            h = "#L{è¯·ä¸Šä¼ 5Mä»¥å†…çš„JPGã€GIFã€PNGå›¾ç‰‡ã€‚}";
                            break;
                        default:
                            h = "#L{ä¸Šä¼ å›¾ç‰‡è¶…æ—¶}"
                    }
